...
handle_savedata_struct :: proc (mode:SAVEGAME_IO, save_data:^init.SaveData, ge:^init.GlobalEverything, ) {
    ...
    order_func := mode == .WRITE ?  mem.copy : reverse_memcopy
...
handle_command :: proc (
    ...
    switch(command){
        case .Save: 
            handle_savedata_struct( .WRITE, &save_data, ge)
            for x, _e in save_data.view_data.scenery_items.enum_state {
                if _e == .CLEARING {
                    fmt.printf("After savewrangling, state is ")
                    for y in x {
                        fmt.printf("%v ", y)
                    }
                    fmt.printf("\n")
            ...
            switch(parse_look_modifier(tokenBuffer[1])){
                ...
                case ._DefaultLook: 
                    ...
                    switch {
                        ...
                        case: str = fmt.aprintf("Cannot find the '%s'.", tokenBuffer[1])
                    }
                }
