...
add_scenery_items_hot :: proc (scenery_items:^SceneryItemData){
    ...
    add_scenery_item(scenery_items, .CLEARING, "STUMP", ..slice.enumerated_array(&vSTUMP_ItemState_descriptions));
...
init_player_item_data_hot :: proc (s:^Item_State) {
    add_playerItem(&s.player_items, .CLEARING, .AXE, ..slice.enumerated_array(&AXE_ItemState_descriptions))
